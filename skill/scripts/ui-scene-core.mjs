...
export function buildPatternStructure({ resolvedPattern, layoutContract = null, bindings = [] }) {
  ...
  const structure = {
    ...
    regions: pattern.regions.map(({ id, width, height, insetOwner, scrollOwner, surface, dividerEdges }) => ({ id, width, height, insetOwner, scrollOwner, surface, dividerEdges })),
...
export function validatePatternBindings({ resolvedPattern, layoutContract = null, bindings = [], requireSlots = false }) {
  const errors = [];
  const pattern = resolvedPattern.pattern;
  ...
  for (const binding of bindings) {
    ...
    if (!slot) {
      errors.push(`${binding.id ?? binding.logicalName ?? "binding"}: unknown Pattern slot '${binding.slot}'`);
      continue;
    }
    if (region && slot.owner !== "page" && slot.owner !== region) errors.push(`${binding.id ?? binding.logicalName ?? "binding"}: slot '${binding.slot}' belongs to '${slot.owner}', not '${region}'`);
    counts.set(binding.slot, (counts.get(binding.slot) ?? 0) + (binding.expectedRuntimeCount ?? binding.count ?? 1));
  }
  if (requireSlots) {
...
export function normalizeUiScene(uiScene, { registry = readPatternRegistry(), layoutContract = null, requireSlots = false } = {}) {
  ...
  const visit = (node) => {
    ...
    ids.add(node.id);
    flattened.push({ id: node.id, logicalName: node.component?.logicalName ?? null, region: node.region, slot: node.slot ?? null, count: 1 });
