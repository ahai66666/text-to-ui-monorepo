...
is retained as a placeholder and the structure may proceed. A missing capture
bundle, stale fingerprint, schema mismatch, missing mapped target or invalid
component contract fails before a Bridge job is created and is never retried.
...
`contentColor` from the execution payload by default; the runtime only applies
