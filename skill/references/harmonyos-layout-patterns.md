...
The Global Title Layer is transparent and structural, never one separate solid bar. The Titlebar component adds no fill. Each shell segment supplies the same surface as the pane directly below it: the Brand segment matches Primary Navigation, the list segment matches Secondary Pane, and the final segment matches Main Content or Main Detail. Do not draw a horizontal divider below the Primary Navigation or Secondary Pane title segments; keep vertical pane dividers continuous through both title and workspace rows.
...
- Expanded mode uses a 40px-high icon + text Primary Button that fills the available width inside the Sidebar's 16px horizontal inset. Its slot uses `--layout-primary-action-slot-gap-top` (8px) below the Brand Anchor / Titlebar and `--layout-primary-action-slot-gap-bottom` (12px) before the first Sidebar navigation component. Scope these tokens to the action slot only; they must not become a whole-navigation padding or a generic menu/component gap. Collapsed 64px mode uses a 40px icon-only Primary Button with an accessible name and Tooltip.
...
- The Global Primary Action Slot sits directly below Brand Anchor and above Sidebar navigation. It exclusively owns the page-global Primary CTA. Expanded and collapsed button behavior follows the Primary Shell Global Primary Slot rules above.
...
- Two-level navigation places the first-level, high-level functional-space entries as icon-only controls at the bottom-left of the navigation region. This is the only approved first-level menu in the client system. Its icons must use source-provided filled/solid glyphs; do not reuse the outline icons used by ordinary navigation or component actions. Unselected icons use the tertiary icon color through `--color-primary-level-unselected` → `--color-icon-subtle` (`--color-neutral-dark-40`); Selected uses the brand foreground on a transparent background. Hover may add the Sidebar accent background but must not change the current icon color. In expanded mode, the icon group fills the Sidebar content width, uses `--size-11` (40px) height, and distributes all controls evenly on the horizontal axis. When the entire navigation is collapsed to 64px, keep every first-level entry visible and stack the 40×40 icon-only controls vertically at the bottom of the navigation region; do not hide unselected entries. The second-level routes continue to use the Sidebar component above them.
