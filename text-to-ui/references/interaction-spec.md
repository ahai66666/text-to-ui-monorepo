...
- **PROJECT:** Every Main Detail pane-global action is placed in the Titlebar `main-detail-actions` slot of the Global Title Layer. The slot accepts 0..n buttons; in a three-pane shell it is the default home for all operations scoped to the complete third pane. The group begins 16px after the Main Detail divider and is ordered by task priority. Actions scoped to a card, field, section, selected object, or inline content remain next to that target; application-wide actions not owned by Main Detail and window controls remain at the far right.
- **PROJECT:** When Search supports an advanced query builder, use the native `advanced-search` trailing slot instead of placing an unrelated button beside the field. The slot accepts 0..1 Ghost Icon Button, defaults to semantic alias `action/filter`, requires an accessible name, and opens the product's advanced-search layer. If the query has a value, render trailing controls in the stable order `clear` then `advanced-search`; the advanced-search control does not replace Clear and does not appear unless the capability is supplied.
