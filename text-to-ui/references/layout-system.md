...
Do not wrap these primary shell regions in decorative cards. Adjacent panes are separated by quiet structural boundaries. The Titlebar is a transparent global title layer, not an independent color band. The component itself adds no fill; each shell segment uses the same surface as the pane directly below it. Primary Navigation and Secondary Pane do not add a horizontal divider below their title segments. Vertical pane dividers remain continuous through the title layer and workspace.
...
- Expanded navigation uses the standard 40px-high Primary Button with icon and text. It fills the available width inside the Sidebar's 16px horizontal inset. The action slot owns an 8px gap below the Brand Anchor / Titlebar through `--layout-primary-action-slot-gap-top`, and a 12px gap before the first Sidebar navigation component through `--layout-primary-action-slot-gap-bottom`. These are scoped slot gaps: do not apply them as padding or direct-child gaps on the whole Primary Navigation, and do not change menu-row spacing. Collapsed 64px navigation keeps a 40px icon-only Primary Button, hides its label, and supplies an accessible name and Tooltip.
