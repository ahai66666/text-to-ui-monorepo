# Text-to-UI Semantic Icon Helper v6 Add

这是一个范围受限的 Pixso 开发插件，用于创建一个语义 SVG 共享组件：
...
4. 插件会检查同名组件、导入 SVG、注册共享组件，并回读确认；回读失败不会再次写入。
...
- 优先使用 `createComponentFromNode`，缺少该能力时尝试 `createComponent + appendChild` 兼容路径。
- 临时 SVG 清理会先检查 `removed`，避免对已经被 Pixso 自动移除的节点再次调用 `remove()`。
