...
<template>
  <header class="tui-component tui-titlebar" data-component="titlebar" data-logical-component="Titlebar/Default" :data-variant="size" :data-size="size" :data-state="disabled ? 'disabled' : state" :data-layout="layout" :data-pane-role="resolvedPaneRole" data-framework="vue">
    ...
    <div v-if="resolvedPaneRole === 'secondary-pane' && resolved.secondaryPaneContent" class="tui-titlebar__secondary-content" data-slot="secondary-pane-content" data-component-slot="registered-component"><Search v-bind="resolved.secondaryPaneContent.props || {}" /></div>
    ...
    <div v-if="resolved.showWindowControls" class="tui-titlebar__actions" data-slot="actions" data-component="titlebar-controls" data-logical-component="Titlebar Controls/Normal" data-size="medium"><Button v-for="[action, text] in actions" :key="action" :label="text" variant="ghost" size="standard" mode="icon" logical-name="Icon Button/Ghost/Default" :icon="`window/${action}`" :icon-size="size === 'small' ? 16 : 24" class="tui-titlebar__action" data-slot="titlebar-action" :data-action="action" data-button-type="icon" :disabled="disabled" @click="emit('action', action)" /></div>
  </header>
</template>
