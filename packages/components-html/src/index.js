...
const renderButton = ({
  ...
  return `<button class="tui-component tui-button${modeClass}" type="button" ${attrs("button", logicalName, variant, resolvedState, ` data-mode="${mode}" data-size="${size}"`)}${typeAttrs}${menuAttrs}${disabledAttr}>${buttonContent({ label, iconName, mode, size, includeChevron: mode === "selection-dropdown" })}</button>`;
};
